...
import { Sparkles, Copy, Check, ChevronDown, ChevronUp, Lightbulb, Maximize2, Minimize2 } from 'lucide-react';
...
export default function CopywritingAssistant({
    ...
    // Determine if this is the mobile instance
    const isMobile = instanceId?.includes('mobile');
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className={`flex-1 flex flex-col overflow-hidden pb-4 ${isMobile ? 'px-6' : ''}`}>
                <div className="flex-1 flex flex-col bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 relative transition-colors gap-4 min-h-0">
                    {/* 結果/編輯區 - 放在最上面 */}
                    <div className="flex-1 flex flex-col min-h-0 relative">
                        <div className="flex items-center justify-between mb-1.5 shrink-0">
                            <label htmlFor={`caption-input-${instanceId}`} className="text-xs font-medium text-gray-500 dark:text-gray-400 block">
                                📝 貼文內容
                            </label>
                            {onToggleExpand && (
                                <button
                        ...
                        {isGenerating ? (
                            <div className="flex-1 bg-white dark:bg-gray-700/30 rounded-lg flex flex-col items-center justify-center text-center border border-gray-200 dark:border-gray-600 border-dashed">
                            ...
                            <div className="flex-1 relative min-h-0">
                                <textarea
                                    ...
                                    className="w-full h-full bg-white dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-lg p-3 resize-none focus:ring-2 focus:ring-blue-500 text-sm text-gray-700 dark:text-gray-200 placeholder-gray-400 leading-relaxed font-sans"
                                />
                                {generatedCaption && (
                                    <button
                                        type="button"
            ...
            {/* Auto Generate Button - Fixed at bottom */}
            <div className={`shrink-0 ${isMobile
                ? 'p-4 bg-white dark:bg-[#1e1e1e] border-t border-gray-100 dark:border-gray-800'
                : 'pt-4' // Desktop: just top spacing, full width
                }`}>
                <button
                    type="button"
                    onClick={onGenerate}
