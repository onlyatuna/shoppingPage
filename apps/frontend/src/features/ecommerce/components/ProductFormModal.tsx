...
import { X, Upload, Trash2, Loader2, Edit, Plus } from 'lucide-react';
...
export default function ProductFormModal({ isOpen, onClose, onSubmit, initialData, isPending }: Props) {
    ...
    const [prevInitialDataId, setPrevInitialDataId] = useState<string | number | undefined>(undefined);

    const currentInitialDataId = initialData?.id;
    const shouldReset = (isOpen && !prevIsOpen) || (isOpen && currentInitialDataId !== prevInitialDataId);

    if (shouldReset) {
        setPrevIsOpen(isOpen);
        setPrevInitialDataId(currentInitialDataId);
    ...
    if (enableVariants !== prevEnableVariants || JSON.stringify(options) !== JSON.stringify(prevOptions)) {
        ...
        if (!enableVariants) {
            ...
            const isInitialOptions = initialData?.options && JSON.stringify(options) === JSON.stringify(initialData.options);
