...
import { Product, Category, ProductOption, ProductVariant } from '@/types/interface_definitions';
...
export default function AdminProductFormPage() {
    ...
    // --- 1. Form Initialization (Render Phase Sync) ---
    const [lastInitializedId, setLastInitializedId] = useState<string | null>(null);

    // Detect when we should re-initialize the form:
    // 1. Initial product data just loaded
    // 2. Editor state just became available
    // 3. We navigated to a different product
    const shouldInit = (product && product.id.toString() !== lastInitializedId) ||
        (editorState && !lastInitializedId && !isEditMode);

    if (shouldInit) {
        setLastInitializedId(product?.id.toString() || 'editor-init');

        // Use a deferred call or direct reset depending on form state
        // In render phase, we can call reset() but must be careful
        reset({
            name: product?.name || '',
            price: product?.price ? Number(product.price) : 0,
            stock: product?.stock || 10,
            description: editorState?.initialDescription || product?.description || '',
            images: editorState?.initialImage || product?.images?.join(',') || '',
            categoryId: product?.categoryId || (categories && categories.length > 0 ? categories[0].id : 0),
            isActive: product?.isActive ?? true,
            slug: product?.slug || (product?.id ? `product-${product.id}` : `new-${Date.now()}`),
        });
    ...
    // --- 2. Variant Generation (Render Phase Sync) ---
    // Auto-generate variants when options change OR when variants are enabled
    const [prevOptions, setPrevOptions] = useState(options);
    const [prevEnableVariants, setPrevEnableVariants] = useState(enableVariants);

    if (enableVariants !== prevEnableVariants || JSON.stringify(options) !== JSON.stringify(prevOptions)) {
        setPrevEnableVariants(enableVariants);
                                                        ...
                                                        <div className={`border border - gray - 200 rounded - lg px - 3 py - 2 bg - white hover: border - gray - 300 transition - all duration - 400 flex items - center justify - between gap - 2 ${isDragging ? 'bg-gray-100 border-gray-300 shadow-lg' : ''} `}>
                                                            <button
                                                                ...
                                                                onDragStart={(e: React.DragEvent) => {
                                                                    ...
                                                                    e.dataTransfer.setData('text/plain', JSON.stringify({ optionIdx: idx, valueIdx: vIdx }));
