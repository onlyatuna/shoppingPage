    ...
    return (
        <div className={`group bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden flex flex-col ${reverse ? 'lg:flex-row-reverse' : 'lg:flex-row'} hover:border-blue-100 transition-colors duration-300`}>
            ...
            >
                {imagePath ? (
                    <div
                        className={`absolute inset-0 ${containImage ? 'bg-contain bg-center bg-no-repeat' : 'bg-cover bg-top'} transition-transform duration-700 group-hover:scale-110`}
                        style={{ backgroundImage: `url(${imagePath})` }}
                    />
                ) : (
                    <div className="absolute inset-0 flex items-center justify-center text-gray-300 font-bold text-xl transition-transform duration-700 group-hover:scale-110">
                        預覽圖片
                    </div>
                )}
