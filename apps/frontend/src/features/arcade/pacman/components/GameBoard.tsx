...
const GhostCharacter: React.FC<{ ghost: Ghost; frightenedTimer: number }> = ({ ghost, frightenedTimer }) => {
...
const GameBoard: React.FC<GameBoardProps> = ({ dots, powerPellets, pacman, ghosts, frightenedTimer }) => {
  return (
    <div className="bg-black border-2 border-blue-600 relative overflow-hidden"
      ...
      {ghosts.map(ghost => (
        <GhostCharacter key={ghost.name} ghost={ghost} frightenedTimer={frightenedTimer} />
      ))}
    </div>
  );
};
